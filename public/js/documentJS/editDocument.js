...
document.getElementById('EditDocumentForm').addEventListener('submit', function (e) {
    ...
    fetch('../../../app/models/modals/EditDocument.php', {
        method: 'POST',
        body: formData
    })
...
function updateTable() {
    const url = '../../../app/models/GetData/getDocumentTable.php';
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (!Array.isArray(data)) {
                console.error("Ошибка: сервер вернул не массив");
                return;
            }
            ...
            data.forEach((doc, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td>${doc.purpose}</td>
                    <td>${doc.docType}</td>
                    <td class="d-flex" style="border-radius: 0px 36px 36px 0px;">
                        <a href="#" class="docViewBtn" data-documentID="${doc.documentID}">
                            <button type="button" class="editEmployerBtn" title="Переглянути документ">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 22" class="icon_white no-click">
                        ...
                        <button type="button" class="editEmployerBtn editPositionBtn" 
                            data-documentID="${doc.documentID}"
                            data-ownerName="${doc.ownerName}"
                            data-documentName="${doc.documentName}"
                            data-sphere="${doc.sphere}"
                            data-purpose="${doc.purpose}"
                            data-docType="${doc.docType}"
                            data-bs-toggle="modal" 
                            data-bs-target="#EditDocumentModal"
                            title="Змінити дані про документ">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon_white no-click">
