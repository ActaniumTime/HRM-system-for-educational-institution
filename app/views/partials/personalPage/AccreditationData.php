    ...
    <table class="table table-hover align-middle">
        ...
        <tbody id="accreditationTable">
            <?php 
                ...
                $Accred->getAccreditationByID($connection, $_SESSION['employer_ID']);
                $counter = 1;
                ...
                $newestYear = $currentYear + 5; 
                    ...
                    for ($i = $oldestYear; $i <= $newestYear; $i++) {
                        if ($Accred->isAccreditedYear($i)) {
                            if($i == $currentYear){
                                echo "<td id=\"currentYear\"> <div class=\"AccredInfo\">" . $Accred->outputCategory($i) . "<div></td>";
                            } else {
                                echo "<td><div class=\"AccredInfo\">" . $Accred->outputCategory($i) . "<div></td>";
                            }
                    ...
                    echo "<button type=\"button\" class=\"editEmployerBtn \" 
                            data-employer-id=\"{$Accred->getEmployerID()}\"
                            data-accreditation-id=\"{$Accred->getAccreditationID()}\"
                            data-accreditation-plan='" . json_encode($Accred->getAccreditationPlan()) . "'
                            data-document-years='" . json_encode($Accred->getDocumentYears()) . "'
                            data-finish-day='" . json_encode($Accred->getFinishDay()) . "'
                            data-expirience-years=\"{$Accred->getExperienceYears()}\"
                            data-emp-name=\"{$emp->getEmpNameByID($Accred->getEmployerID())}\"
                            data-emp-cat=\"{$Accred->currentCategory()}\"
